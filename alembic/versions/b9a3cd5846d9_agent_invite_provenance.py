...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_agents_invited_by_agent_id"),
        "agents",
        ["invited_by_agent_id"],
        unique=False,
    )
    ...
    op.add_column(
        "invite_tokens", sa.Column("created_by_agent_id", sa.UUID(), nullable=True)
    )
    op.create_index(
        op.f("ix_invite_tokens_created_by_agent_id"),
        "invite_tokens",
        ["created_by_agent_id"],
        unique=False,
    )
    op.create_foreign_key(
        "fk_invite_tokens_created_by_agent_id",
        "invite_tokens",
        "agents",
        ["created_by_agent_id"],
        ["id"],
...
def downgrade() -> None:
    """Downgrade schema."""
    op.drop_constraint(
        "fk_invite_tokens_created_by_agent_id", "invite_tokens", type_="foreignkey"
    )
    op.drop_index(
        op.f("ix_invite_tokens_created_by_agent_id"), table_name="invite_tokens"
    )
